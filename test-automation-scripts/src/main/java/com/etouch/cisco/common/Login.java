...
public class Login extends BaseTest {
	...
	@ITafExcelDataProviderInputs(excelFile = "file1", excelsheet = "CISCO", dataKey = "AuthenticationFail")
	public void invalidTestSignIn(TestParameters inputs) throws PageException {
		try {
			System.out.println("Starting test case 2");
			mainPage.Logout(inputs);
			mainPage.InvalidSignIn(inputs);
			Thread.sleep(1000);
			String actualURL = mainPage.getPageUrl();
			System.out.println("The expected URL: "
					+ inputs.getParamMap().get("expected")
